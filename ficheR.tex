...
\hypersetup{linkcolor=black, colorlinks=TRUE, urlcolor=cyan }

\usepackage{suffix} %pour la version étoilée de grasindex
...
\newif\ifadvanced
\advancedtrue  % set \advancedtrue pour la fiche avancée \advancedfalse pour la fiche simple

\makeindex
\newcommand{\grasindex}[1]{\textbf{#1} \index{#1\textbf}}
%\newcommand{\grasindex}[1]{\textit{#1} \index{#1\textbf}}
...
R contient plusieurs types d'objets qui appartiennent à des classes\ref{class} et des modes\ref{mode} différents. Pour plus d'informations: le site \href{http://r-pkgs.had.co.nz/}{R advanced} d'Hadley Wickham.

\subsection{Répertoire de travail}

Le répertoire de travail est différent de les environnements\ref{EnvSesDeb} de R. 
...
\grasindex*{class}(obj) \label{class} \quad donne la classe de l'objet obj. 5 classes principales: \grasindex{vector}, \grasindex{factor}, \grasindex{matrix} (un seul type de colonne), \grasindex{dataframe} (plusieurs types de colonne) et \grasindex{list}.

\grasindex*{mode}(obj) \label{mode} \quad donne le mode de l'objet obj. 5 modes principaux: "integer \grasindex{numeric}, \grasindex{character}, \grasindex{logical}, \grasindex{complex} et \grasindex{raw}.
...
\grasindex*{str}(obj) \quad donne la structure interne de l'objet obj de façon concise. Très utile dans le cas d'objet complexe
...
\grasindex*{head}(a, x)  \quad affiche les x premières lignes de la matrice
...
\grasindex*{nrow}(nomdefichier)  \quad nombre de lignes du fichier

\grasindex*{ncol}(nomdefichier)  \quad nombre de colonnes du fichier

\grasindex*{rownames}(a)\textless{}-rownames(b)  \quad on renomme les lignes de a comme les lignes de b

\grasindex*{colnames}(a)\textless{}-colnames(b)  \quad on renomme les colonnes de a comme les colonnes de b

\grasindex*{dim}(a) donne les dimensions du vecteur  \quad \textbar{} d'une matrice
...
X\textless{}-\grasindex*{c}(a, b, c, d)  \quad affecte les valeurs au vecteur X , \grasindex{c}= concatener

Y\textless{}-\grasindex{c}(1\grasindex*{:}10)  \quad affecte valeurs entière de 1 à 10
...
\grasindex*{order}(a) \quad  donne les coordonnées du plus petit élément de a puis du deuxième, ect... 
...
\textbf{a\grasindex*{\$}nomcol1} \quad affiche les éléments de a de la colonne nommé nomcol1, équivalent à \textbf{a[, nomcol1]=X} 
...
\grasindex*{subset}(a, X!=0)  \quad donne une sous division de la matrice (ou d'un df) a pour laquelle la 1ère colonne est différente de 0; l'option \textsc{select} permet de sélectionner les colonnes concernées. 
...
\grasindex*{split}(df, fact)  \quad sépare df selon les modalités de fact
...
\grasindex*{levels}(fact)  \quad donne les modalités du facteur fact

\grasindex*{table}(fact)  \quad retourne une table des valeurs. \grasindex{table}(fact1, fact2) donne la matrice d'association entre les deux facteurs 
...
\grasindex*{list}(obj1,obj2) \quad  produit une liste avec les objets
...
\grasindex*{mean}(X)  \quad moyenne de X
...
\grasindex*{sd}(X)  \quad écart type de X

\grasindex{var}(X) \quad  variance de X

\grasindex*{max}(X)  \quad valeur max de X \adv{pmax est la version vectorisée de max}

\grasindex*{min}(X)  \quad valeur min de X \adv{pmin est la version vectorisée de max}

\grasindex{summary}(X)  \quad quartiles, médiane, moyenne et valeurs extrèmes du vecteur

\grasindex*{length}(X) \quad  taille d'un vecteur
...
m\textless{}-\grasindex*{lm}(x\texttildelow y)  \quad donne un modèle linéaire
...
\grasindex*{rnorm}(x, 0, 1)  \quad donne x valeurs tirées au hasard  d'une loi normale centrée réduite suivant µ=0 et var=1
...
\grasindex*{sample}(X)  \quad permutation aléatoire des élément du vecteur x. L'argument \textsc{replace} permet de choisir un tirage avec ous sans remise.
...
\grasindex*{plot}(X \texttildelow Y, option cf plus bas)  \quad affiche le diagramme de dispersion de X en fonction de la Y

\grasindex*{points}(abscisse, ordonnée)  \quad pour rajouter des points sur un même graphique

\grasindex*{lines}(X \texttildelow Y)  \quad rajoute une ligne sur le graphique courant

\grasindex*{abline}(h=x)  \quad ajoute sur la fenêtre graphique une ligne horizontal (v=x pour une ligne vertical) d'ordonné x
...
\grasindex*{rgb}(0,0,1,0.5)  \quad donne une couleur avec le premier chiffre (entre 0 et 1) donne la quantité de rouge, le deuxième la quantité de vert, le troisème de bleu et le dernier donne l'opacité. Ici il s'agit d'un bleu moyennement transparent 
...
\grasindex*{par()} \quad  renvoie la liste de tous les paramètres. \textbf{oldpar \textless{}- par()} : sauvegarde le paramétrage courant dans la variable oldpar, \textbf{par(cex = oldpar\$cex)}   rétablit le paramètre cex à la valeur précédemment sauvegardée dans oldpar.
...
\grasindex*{for}\textbf{(i in 1 : x) \{ commandes\}}  \quad formation d'une boucle de pas 1 qui va de 1 au nombre x et qui applique les commandes à chaque boucle 
...
\grasindex*{if}(condition)  \{action1\}   \grasindex{else}  \{action2\}  \quad si la condition est respecté-\textgreater{}action1 sinon -\textgreater{}action 2

\grasindex*{ifelse}(test, ``oui'', ``non'')  \quad Version vectorisée de \textsc{if} fait un test renvoie la première valeur si test vrai (ici ``oui'') et deuxième valeur si test faux (ici ``non'')
...
\grasindex*{traceback}() \quad donne la séquence de commandes qui a amené à la dernière erreur
...
\grasindex*{\&} \quad  ET logique 

\grasindex*{\textbar{}}  \quad (alt-gr 6) OU logique
...
\grasindex*{rbind}(X, Y) et \grasindex*{cbind}((X, Y))  \quad fusionne les deux vecteurs en une matrice à 2 lignes (ou 2 colonnes); peut s'appliquer avec une matrice
...
\grasindex*{\#}  \quad après un dièse ont peut ajouter un commentaire dans le script. Toutes les lettres après ce signe seront ignorer par R

\grasindex{!} \quad non

\grasindex*{!=} \quad différent 
